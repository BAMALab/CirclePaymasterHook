...
import { estimateUserOperationGas as estimateUserOperationGas2 } from "viem/account-abstraction";
...
async function estimateExecuteRecoveryGas(client, params) {
  ...
  return await estimateUserOperationGas2(client, {
    callData,
    ...userOp
  });
}
...
import { encodeFunctionData as encodeFunctionData3, RpcError } from "viem";
import { sendUserOperation } from "viem/account-abstraction";
import { parsePublicKey as parsePublicKey2 } from "webauthn-p256";
...
async function executeRecovery(client, params) {
  ...
  const { credential, ...userOp } = params;
  const publicKeyOwner = parsePublicKey2(credential.publicKey);
  try {
    ...
    const addressMappingAlreadyExistsError = error instanceof RpcError && error.code === -32602 /* ALREADY_KNOWN */;
...
import { encodeFunctionData as encodeFunctionData4, RpcError as RpcError2 } from "viem";
import { sendUserOperation as sendUserOperation2 } from "viem/account-abstraction";
async function registerRecoveryAddress(client, params) {
  ...
  try {
    ...
    const addressMappingAlreadyExistsError = error instanceof RpcError2 && error.code === -32602 /* ALREADY_KNOWN */;
    if (!addressMappingAlreadyExistsError) {
      throw new Error(
  ...
  return await sendUserOperation2(client, {
    callData,
    ...userOp
  });
}
...
import { custom as custom2 } from "viem";
var toPasskeyTransport = (clientUrl, clientKey) => {
  ...
  return custom2(provider, config);
};
...
import { parsePublicKey as parsePublicKey3 } from "webauthn-p256";
function getPublicKeyParamsFromOwner(owner) {
  const publicKey = parsePublicKey3(owner.publicKey);
...
function getSenderForContract(owner) {
  return isWebAuthnOwner(owner) ? getPublicKeyParamsFromOwner(owner).sender : pad2(owner.address);
}

// src/utils/address/initializeUpgradableMSCA.ts
import { encodeAbiParameters as encodeAbiParameters3, encodeFunctionData as encodeFunctionData5 } from "viem";
...
function getPluginInstallParams(owner) {
  return isWebAuthnOwner(owner) ? encodeAbiParameters2(CIRCLE_PLUGIN_INSTALL_DATA_ABI, [
    [],
    [],
    getPublicKeyParamsFromOwner(owner).initialPublicKeyOwners,
    OWNER_WEIGHTS,
    THRESHOLD_WEIGHT
  ]) : encodeAbiParameters2(CIRCLE_PLUGIN_INSTALL_DATA_ABI, [
    [owner.address],
    OWNER_WEIGHTS,
    [],
    [],
...
function getInitializeUpgradableMSCAParams(owner) {
  const pluginInstallParams = getPluginInstallParams(owner);
  return encodeAbiParameters3(INITIALIZING_DATA_ABI_PARAMS, [
    [CIRCLE_WEIGHTED_WEB_AUTHN_MULTISIG_PLUGIN.address],
    [CIRCLE_WEIGHTED_WEB_AUTHN_MULTISIG_PLUGIN.manifestHash],
    [pluginInstallParams]
  ]);
...
function computeAddress(owner) {
  ...
  const bytecode = encodePacked(
    ...
    [
      ...
      encodeAbiParameters4(parseAbiParameters("address, bytes"), [
...
import {
  ...
  bytesToCryptoKey,
  cryptoKeyToBytes,
  parsePublicKey as parsePublicKey4
} from "webauthn-p256";
async function parseBase64EncodedPublicKey(base64Url) {
  ...
  return parsePublicKey4(publicKey);
}
...
function toReplaySafeHash({
  ...
  const domainSeparatorTypeHash = keccak2563(
    encodePacked2(["string"], [domainSeparatorType])
  );
  const moduleNameHash = keccak2563(encodePacked2(["string"], [name]));
  const moduleVersionHash = keccak2563(encodePacked2(["string"], [version]));
  const moduleTypeHash = keccak2563(encodePacked2(["string"], [moduleType]));
  const domainSeparator = keccak2563(
    encodeAbiParameters5(
      [
        { type: "bytes32" },
        { type: "bytes32" },
        { type: "bytes32" },
      ...
      [
        ...
        BigInt(chainId),
        CIRCLE_WEIGHTED_WEB_AUTHN_MULTISIG_PLUGIN.address,
        pad3(address, { dir: "right" })
      ]
    )
  );
  const structHash = keccak2563(
    encodeAbiParameters5(
      [{ type: "bytes32" }, { type: "bytes32" }],
      [moduleTypeHash, hash]
    )
  );
  return keccak2563(concatHex([EIP712_PREFIX, domainSeparator, structHash]));
}

// src/utils/signature/toWebAuthnSignature.ts
import { encodeAbiParameters as encodeAbiParameters6, stringToHex } from "viem";
import {
...
function toWebAuthnSignature({
  ...
  return encodeAbiParameters6(AUTHENTICATOR_DATA, [
    {
      webAuthnData: {
...
import { encodePacked as encodePacked4, parseSignature } from "viem";
function wrapEoaSignature({
  ...
  if (typeof v !== "undefined") {
    ...
    return encodePacked4(["bytes32", "bytes32", "uint8"], [r, s, signType]);
  }
...
async function toCircleSmartAccount(parameters) {
  ...
  return toSmartAccount({
    ...
    async getFactoryArgs() {
      const factoryData = encodeFunctionData7({
        abi: FACTORY.abi,
    ...
    async signMessage(parameters2) {
      ...
      const hash = toReplaySafeHash({
        address: address2,
        chainId: client.chain.id,
        hash: hashMessage2(parameters2.message)
      });
      return signAndWrap({
        hash,
        owner,
    ...
    async signUserOperation(parameters2) {
      ...
      const pubKeyId = pad4(slice(sender, 2));
      return signAndWrap({
...
function getCredentialCreationOptions(options) {
  return {
    ...options,
    challenge: base64UrlToBytes2(options.challenge),
    user: {
      ...options.user,
      id: base64UrlToBytes2(options.user.id)
    }
  };
}
function getCredentialRequestOptions(options) {
  return {
    ...
    allowCredentials: options.allowCredentials?.map((credential) => ({
      ...credential,
      id: base64UrlToBytes2(credential.id)
    }))
  };
}
export {
